...
class ImgUI:
    ...
    def drawing_effect(self, event):
        if self.do_capture:
            ...
            if toggle == 0:  # print circles
                cv2.circle(res, (int(event.x * (res.shape[1] / width)), int(event.y * (res.shape[0] / height))),
                           int(paint_size * (res.shape[0] / height)), draw_color, -1)
            elif toggle == 1:
                cv2.rectangle(res, (int(event.x * (res.shape[1] / width)), int(event.y * (res.shape[0] / height))), (
                    int(event.x * (res.shape[1] / width)) + int(paint_size * 2 * (res.shape[0] / height)),
                    int(event.y * (res.shape[0] / height)) + int(paint_size * 2 * (res.shape[0] / height))), draw_color, -1)
            else:
                cv2.circle(res, (int(event.x * (res.shape[1] / width)), int(event.y * (res.shape[0] / height))),
                           int(paint_size * 2 * (res.shape[0] / height)), draw_color, int(1 * (res.shape[0] / height)))
    ...
    def do_zoom(self, new_value):
        ...
        print(self.img.size)
        print((int(size[0] * new_value), int(size[1] * new_value)))
        resize_image = self.img.resize((int(size[0] * new_value), int(size[1] * new_value)))
        # max shift w szerokosci to 600
        # max shift w wysokosci to 450

        # 800x600 - orginał obrazek
        # 1200x800 - zlupowany
        #
